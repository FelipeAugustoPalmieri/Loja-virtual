...
class ModelAccountCustomer extends Model {
	...
	public function editIdAsaas($idasaas){
		if($this->customer->isLogged()){
			$this->db->query("UPDATE " . DB_PREFIX . "customer SET id_asaas = '" . $this->db->escape($idasaas) . "' WHERE customer_id = '" . (int)$this->customer->getId() . "'");
		}
	}
	...
	public function addCustomerAjax($data) {
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "customer SET customer_group_id = '" . (int)$customer_group_id . "', store_id = '" . (int)$this->config->get('config_store_id') . "', language_id = '" . (int)$this->config->get('config_language_id') . "', firstname = '" . $this->db->escape($data['consumidor']['name']) . "', logintbest = '" . $this->db->escape($data['consumidor']['identifier']) . "', custom_field = '" . json_encode(array("2"=>$this->db->escape($data['consumidor']['national_identifier']), "3"=>$this->db->escape($data['physicalperson']['born_on']))) . "', senhatbest = '" . $this->db->escape($data['senha']) . "', email = '" . $this->db->escape($data['legalperson']['email']) . "', telephone = '" . $this->db->escape($data['legalperson']['cell_number']) . "', salt = '" . $this->db->escape($salt = token(9)) . "', password = '" . $this->db->escape(sha1($salt . sha1($salt . sha1($data['senha'])))) . "', newsletter = '1', ip = '" . $this->db->escape($this->request->server['REMOTE_ADDR']) . "', status = '" . (int)!$customer_group_info['approval'] . "', date_added = NOW()");
	...
	public function getCustomerTbest($identifier) {
			$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer WHERE logintbest = '" . $identifier . "'");
			return $query->row;
	}
	...
	public function getTotalCustomersByCpf($cpf) {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "customer WHERE cpf = '" . $this->db->escape(utf8_strtolower($cpf)) . "'");
