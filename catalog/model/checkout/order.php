...
class ModelCheckoutOrder extends Model {
	...
	public function editTotalsOrder($order_id, $data){
		// Totals
	...
	public function getOrderSubTotal($order_id) {
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "order_total` WHERE order_id = '" . (int)$order_id . "' AND code = 'sub_total' ORDER BY sort_order ASC");
		
		return $query->rows;
	}	

	public function addOrderIdAsaas($order_id, $id_asaas){
		$query = $this->db->query("UPDATE `" . DB_PREFIX . "order` SET id_asaas = '" . $id_asaas . "' WHERE order_id = ".$order_id);
	}
	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $override = false) {
		...
		if ($order_info) {
			...
			$subtotal = $this->getOrderSubTotal($order_id);
			...
			if (!in_array($order_info['order_status_id'], array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status'))) && in_array($order_status_id, array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status')))) {
				...
				if((float)$subtotal[0]['value'] > 0){
					$this->customer->login($customer_info['email'], "", true);
					$this->load->model('tool/points');
					//$pontuacao = $this->model_tool_points->regiterPointTbest($order_id, (float)$subtotal[0]['value']);
